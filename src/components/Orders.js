...
const Orders = () => {
  ...
  useEffect(() => {
    fetch(`https://vass-motors.herokuapp.com/orders`)
      .then((res) => res.json())
      .then((data) => {
        setOrders(data);
        setLoading(false);
  ...
  const handleStatusChange = (id, status) => {
    ...
    fetch("https://vass-motors.herokuapp.com/updateOrderStatus", {
      method: "put",
  ...
  return (
    <div className="px-2  mx-md-2 bg-white container" style={{ borderRadius: "15px" }}>
      <h3 className="text-center mb-4 fw-bold">Manage all orders</h3>
      {loading ? (
        <div className="text-center my-1 private-spinner py-1">
